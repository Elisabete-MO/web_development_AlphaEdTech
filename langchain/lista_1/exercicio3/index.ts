...
async function main() {
  const SYSTEM_MESSAGE = "Você é um assistente que responde de forma muito breve.";
  const HUMAN_MESSAGE = "Olá, a conexão está funcionando?";
  ...
  try {
    ...
    console.log("----------------------------------");
    console.log("Enviando mensagens estruturadas...");

    const resposta_list = await model.invoke([
      new SystemMessage(SYSTEM_MESSAGE),
      new HumanMessage(HUMAN_MESSAGE)
    ]);

    console.log(`System message: ${SYSTEM_MESSAGE}`);
    console.log(`Human message: ${HUMAN_MESSAGE}`);
    console.log("\n✅ Resposta do modelo:");
    console.log(resposta_list.content);
  } catch (erro) {
