export function validateOrderBody(req, res, next) {
  ...
  if (id === undefined || customerId === undefined || items === undefined) {
    return res.status(400).json({ message: "id, customerId e items são obrigatórios" });
  }
  ...
  if (typeof customerId !== "number" || !Number.isInteger(customerId)) {
    return res.status(400).json({ message: "customerId deve ser um número inteiro" });
  }
